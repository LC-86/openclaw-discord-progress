...
- 失败时才额外保留必要的提示信息
- 默认 `strict`（严格模式，意思是：只有像任务的消息才出卡）会压住闲聊、吐槽和短讨论
- 如果后续真的进入工具调用或明确执行链路，卡片仍然会自动激活
...
- `auto`
  比 `strict` 更积极一些，适合希望多显示一点任务过程的人。
- `verbose`
  几乎所有运行都会显示卡片。
- `off`
  关闭 Discord 进度卡。
...
- 先发一句闲聊，例如：`算了，先这样吧`
- 确认不会生成任务卡
- 再发一句明确执行请求，例如：`请检查今天上海天气并给我总结`
- 确认会生成 1 张主卡，并在完成后冻结为报告卡
