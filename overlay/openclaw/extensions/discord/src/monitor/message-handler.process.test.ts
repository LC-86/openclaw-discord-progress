...
type DispatchInboundParams = {
  ...
  replyOptions?: {
    onAgentRunStart?: (runId: string) => Promise<void> | void;
...
describe("processDiscordMessage progress sync", () => {
  it("forwards agent lifecycle callbacks into the Discord progress sync adapter", async () => {
    dispatchInboundMessage.mockImplementationOnce(async (params?: DispatchInboundParams) => {
      await params?.replyOptions?.onAgentRunStart?.("run-progress-1");
      await params?.replyOptions?.onReasoningStream?.();
      await params?.replyOptions?.onToolStart?.({ name: "exec" });
      await params?.replyOptions?.onAssistantMessageStart?.();
      await params?.dispatcher.sendFinalReply({ text: "任务完成" });
      return { queuedFinal: true, counts: { final: 1, tool: 0, block: 0 } };
    });
    ...
    expect(progressSyncMocks.current.onAgentRunStart).toHaveBeenCalledWith("run-progress-1");
    expect(progressSyncMocks.current.onReasoningStream).toHaveBeenCalledTimes(1);
    expect(progressSyncMocks.current.onToolStart).toHaveBeenCalledWith({ name: "exec" });
    expect(progressSyncMocks.current.onAssistantMessageStart).toHaveBeenCalledTimes(1);
    expect(progressSyncMocks.current.onFinalReply).toHaveBeenCalledWith({ text: "任务完成" });
    expect(progressSyncMocks.current.finish).toHaveBeenCalledWith({
      aborted: false,
      error: undefined,
    });
  });
