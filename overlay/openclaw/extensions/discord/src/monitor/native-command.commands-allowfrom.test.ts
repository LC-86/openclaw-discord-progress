...
const progressSyncMocks = vi.hoisted(() => {
  const current = {
    onAgentRunStart: vi.fn(async () => {}),
    onReasoningStream: vi.fn(async () => {}),
    onToolStart: vi.fn(async () => {}),
    onAssistantMessageStart: vi.fn(async () => {}),
    onCompactionStart: vi.fn(async () => {}),
    onCompactionEnd: vi.fn(async () => {}),
    onFinalReply: vi.fn(async () => {}),
    finish: vi.fn(async () => {}),
  };
  return {
    current,
    createDiscordProgressSync: vi.fn(() => current),
  };
});

vi.mock("./progress-sync.js", () => ({
  createDiscordProgressSync: progressSyncMocks.createDiscordProgressSync,
}));
...
describe("Discord native slash commands with commands.allowFrom", () => {
  beforeEach(() => {
    vi.restoreAllMocks();
    progressSyncMocks.createDiscordProgressSync.mockClear();
    progressSyncMocks.current.onAgentRunStart.mockClear();
    progressSyncMocks.current.onReasoningStream.mockClear();
    progressSyncMocks.current.onToolStart.mockClear();
    progressSyncMocks.current.onAssistantMessageStart.mockClear();
    progressSyncMocks.current.onCompactionStart.mockClear();
    progressSyncMocks.current.onCompactionEnd.mockClear();
    progressSyncMocks.current.onFinalReply.mockClear();
    progressSyncMocks.current.finish.mockClear();
  });
  ...
  it("forwards slash command agent progress into the Discord progress sync adapter", async () => {
    const cfg = createConfig();
    const command = createCommand(cfg);
    const interaction = createInteraction();
    interaction.client = { rest: {} };
    vi.spyOn(pluginCommandsModule, "matchPluginCommand").mockReturnValue(null);
    vi.spyOn(dispatcherModule, "dispatchReplyWithDispatcher").mockImplementationOnce(
      async (params) => {
        await params.replyOptions?.onAgentRunStart?.("run-slash-1");
        await params.replyOptions?.onReasoningStream?.({ text: "thinking" } as never);
        await params.replyOptions?.onToolStart?.({ name: "web_search", phase: "start" });
        await params.replyOptions?.onAssistantMessageStart?.();
        await params.dispatcherOptions.deliver({ text: "slash done" }, { kind: "final" });
        return {
          counts: {
            final: 1,
            block: 0,
    ...
    expect(progressSyncMocks.createDiscordProgressSync).toHaveBeenCalledTimes(1);
    expect(progressSyncMocks.current.onAgentRunStart).toHaveBeenCalledWith("run-slash-1");
    expect(progressSyncMocks.current.onReasoningStream).toHaveBeenCalledTimes(1);
    expect(progressSyncMocks.current.onToolStart).toHaveBeenCalledWith({
      name: "web_search",
      phase: "start",
    });
    expect(progressSyncMocks.current.onAssistantMessageStart).toHaveBeenCalledTimes(1);
    expect(progressSyncMocks.current.onFinalReply).toHaveBeenCalledWith({ text: "slash done" });
    expect(progressSyncMocks.current.finish).toHaveBeenCalledWith({ error: undefined });
  });
});
