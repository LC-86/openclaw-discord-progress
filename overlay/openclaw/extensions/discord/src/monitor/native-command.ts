...
import { createDiscordProgressSync } from "./progress-sync.js";
...
async function dispatchDiscordCommandInteraction(params: {
  ...
  const interactionRest =
    interaction.client &&
    typeof interaction.client === "object" &&
    "rest" in interaction.client &&
    interaction.client.rest
      ? interaction.client.rest
      : undefined;
  const progressSync =
    rawChannelId && interactionRest
      ? createDiscordProgressSync({
          cfg,
          accountId: effectiveRoute.accountId,
      ...
      : noopDiscordProgressSync;

  let didReply = false;
  let dispatchError: unknown;
  const dispatchResult = await (async () => {
    try {
      return await dispatchReplyWithDispatcher({
        ctx: ctxPayload,
        cfg,
        dispatcherOptions: {
          ...replyPipeline,
          ...
          deliver: async (payload) => {
            ...
            await progressSync.onFinalReply(payload);
            if (suppressReplies) {
              return;
            }
            try {
        ...
        replyOptions: {
          ...
          onAgentRunStart: async (runId) => {
            await progressSync.onAgentRunStart(runId);
          },
          onReasoningStream: async () => {
            await progressSync.onReasoningStream();
          },
          onAssistantMessageStart: async () => {
            await progressSync.onAssistantMessageStart();
          },
          onToolStart: async (payload) => {
            await progressSync.onToolStart(payload);
          },
          onCompactionStart: async () => {
            await progressSync.onCompactionStart();
          },
          onCompactionEnd: async () => {
            await progressSync.onCompactionEnd();
          },
          onModelSelected,
        },
      });
