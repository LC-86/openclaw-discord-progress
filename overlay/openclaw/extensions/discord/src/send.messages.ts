...
import type { APIChannel, APIEmbed, APIMessage } from "discord-api-types/v10";
...
export async function editMessageDiscord(
  ...
  const embeds = Array.isArray(payload.embeds)
    ? payload.embeds.map((embed) => (embed instanceof Embed ? embed : new Embed(embed as APIEmbed)))
    : undefined;
  ...
  const body = {
    ...
    ...(serialized.embeds ? { embeds: serialized.embeds } : {}),
    ...(serialized.components ? { components: serialized.components } : {}),
  };
