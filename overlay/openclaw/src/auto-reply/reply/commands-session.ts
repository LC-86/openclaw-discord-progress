...
const PROGRESS_DISPLAY_MODES = ["strict", "auto", "verbose", "off"] as const;
let cachedChannelRuntime: ReturnType<typeof createPluginRuntime>["channel"] | undefined;

type ProgressDisplayMode = (typeof PROGRESS_DISPLAY_MODES)[number];
...
function normalizeProgressDisplayMode(raw: string | undefined): ProgressDisplayMode | undefined {
  const normalized = raw?.trim().toLowerCase();
  return PROGRESS_DISPLAY_MODES.find((mode) => mode === normalized);
}

function resolveConfiguredProgressMode(cfg: unknown): ProgressDisplayMode {
  const channels = isRecord(cfg) && isRecord(cfg.channels) ? cfg.channels : undefined;
  const discord = isRecord(channels?.discord) ? channels.discord : undefined;
  const progress = isRecord(discord?.progress) ? discord.progress : undefined;
  const configured = normalizeProgressDisplayMode(
    typeof progress?.mode === "string" ? progress.mode : undefined,
  );
  const fromEnv = normalizeProgressDisplayMode(process.env.OPENCLAW_DISCORD_PROGRESS_MODE);
  return configured ?? fromEnv ?? "strict";
}

function setConfiguredProgressMode(cfg: Record<string, unknown>, mode: ProgressDisplayMode): void {
  const channels = isRecord(cfg.channels) ? cfg.channels : {};
  cfg.channels = channels;
  const discord = isRecord(channels.discord) ? channels.discord : {};
  channels.discord = discord;
  const progress = isRecord(discord.progress) ? discord.progress : {};
  discord.progress = progress;
  progress.mode = mode;
}
...
export const handleProgressCommand: CommandHandler = async (params, allowTextCommands) => {
  if (!allowTextCommands) {
    return null;
  }
  const normalized = params.command.commandBodyNormalized;
  if (normalized !== "/progress" && !normalized.startsWith("/progress ")) {
    return null;
  }
  if (!params.command.isAuthorizedSender) {
    logVerbose(
      `Ignoring /progress from unauthorized sender: ${params.command.senderId || "<unknown>"}`,
    );
    return { shouldContinue: false };
  }

  const rawArgs = normalized === "/progress" ? "" : normalized.slice("/progress".length).trim();
  if (!rawArgs) {
    const current = resolveConfiguredProgressMode(params.cfg);
    return {
      shouldContinue: false,
      reply: {
        text:
          `⚙️ 当前 Discord 任务进度模式：${current}\n` +
          "用法：/progress strict|auto|verbose|off\n" +
          "说明：strict=只对明显任务显示；auto=自动判断；verbose=更积极显示；off=关闭卡片。",
      },
    };
  }

  const requested = normalizeProgressDisplayMode(rawArgs);
  if (!requested) {
    return {
      shouldContinue: false,
      reply: {
        text:
          "⚠️ 无效模式。可选值：strict、auto、verbose、off。\n" +
          "说明：strict=只对明显任务显示；auto=自动判断；verbose=更积极显示；off=关闭卡片。",
      },
    };
  }
  ...
  if (!snapshot.valid || !snapshot.parsed || typeof snapshot.parsed !== "object") {
    return {
      shouldContinue: false,
      reply: {
        text: "⚠️ 当前配置文件无效，先修复配置后才能切换 progress 模式。",
      },
    };
  }

  const parsedBase = structuredClone(snapshot.parsed as Record<string, unknown>);
  setConfiguredProgressMode(parsedBase, requested);
  ...
  if (!validated.ok) {
    ...
    return {
      shouldContinue: false,
      reply: {
        text: `⚠️ progress 模式保存失败：${issue.path} ${issue.message}`,
      },
    };
  }
  ...
  return {
    ...
    reply: {
      text:
        `⚙️ Discord progress 模式已切换为 ${requested}。\n` +
        "说明：strict=只对明显任务显示；auto=自动判断；verbose=更积极显示；off=关闭卡片。",
    },
  };
};
